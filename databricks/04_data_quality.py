...
def add_check(dataset, rule, status, checked, failed, severity="ERROR", detail=""):
    checks.append(
        (run_id, dataset, rule, status, int(checked), int(failed), severity, detail)
    )
# COMMAND ----------
...
invalid_status = fact_order.filter(
    ~F.col("status").isin("COMPLETED", "CANCELLED", "RETURNED", "PENDING")
).count()
add_check(
    "gold.fact_order",
    "status_domain",
...
add_check(
    ...
    "net_amount_non_negative",
    "PASS" if negative_net == 0 else "FAIL",
    sales_rows,
    negative_net
)
...
add_check(
    "gold.fact_sales_line",
    "product_fk_present",
    "PASS" if null_product == 0 else "FAIL",
    sales_rows,
    null_product
)
# COMMAND ----------
